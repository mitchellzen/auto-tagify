...
class tagify():
  def __init__(self,text='',link_path='',tags_only=False):
    ...
    try:
      if tags_only:
        self._get_tag_list(text,stop_word,clean_word)
      else:
        self._get_tagged_result(text,link_path,stop_word,clean_word)
    except:
      return 'Invalid text: could not be processed'
      
  def _get_tagged_result(self,text,link_path,stop_word,clean_word):
    tag_words = ''
  ...
  def _get_tag_list(self,text,stop_word,clean_word):
    tag_words = []
    tag_count_list = {}
